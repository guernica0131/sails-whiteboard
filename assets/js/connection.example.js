/* 
* connection.example.js * * This file contains some example, browser - side JavaScript
    for connecting * a client socket(i.e.browser tab) to your Sails backend using Socket.io.*It exposes a global object, `socket`, that you can * * Depending on your useful
    case, this may or may not be the right approach.*This file is designed to get you up and running fast, but it 's just an example.
...
socket.on('connect', function socketConnected() {
    ...
    socket.get('/api/v1/files', function(files) {
        console.log("Now listening to the socket", files);

        socket.on('file', function(message) {
            console.log("I just got a new MODEL", message);
        });
    })
